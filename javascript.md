...
3. 공백 병합
    ...
    3. 선언식에서의 공백 병합
    4. 메서드 체이닝
    5. 주의 : ''작은 따옴표, ""큰 따옴표는 개행 공백을 인식하지 못함 -> ``(백틱, 팀프릴 리터럴)은 공백을 인식함
    ...
    -   const(ES5) : 블록 레벨 스코프, 재선언시 애러 O, 콘솔에서는 애러 X, 변경이 불가능한 자료형(상수)
        * 모든 변수는 const를 사용하고, let을 꼭 사용해야 하는 변수에서만 let을 사용하길 권고합니다.
        ...
        function hello() {
            ...
            // 1000줄의 코드
            return x * 1000;
        } // let인 경우 바뀔 확률이 있으므로 모든 변수를 const 사용하는 것이 좋다.
        ```
        * DOM을 탐색해야 하는 경우 const를 주로 사용하고, (간혹) $하는 경우도 있습니다. 반복문이나 함수 밖에서 주로 사용합니다.
        ```js
...
-   비교 연산자(>, >=, <, <=, ==, !=, ===, !==)
    - 문자열 비교 : 유니코드 순
        - 문자열 순서대로 하나씩 비교하고, 문자가 모두 같은데 길이가 다르 경우 길이가 더 긴 것이 true (Bee > Be)
...
-   BigInt
    ...
    ```js
    const bigInt = 1234567890123412415678901234345671234568901234567345890n;
    ```
...
-   undefine : undefind (값이 할당되지 않은 상태)

    -   형태 : let a, console.log(a)

-   null : object (어느 자료형에서 속하지 않음 - 오류임) (값이 비어있음을 명시)
...
-   명시적 형변환이 아니라 암시적(묵시적) 형변환
    ```js
    console.log("6"/"2"); // 3 숫자형
    console.log("5" * 2); // 10 숫자형
    console.log("5" * "2"); // 10 숫자형
    console.log(+"6"); // 6 숫자형
    console.log(-"6"); // -6 숫자형
    ...
    ```
-   0, null, undefined, NaN, ""은 false이며 그 외의 값은 true 입니다.
-   +를 사용해서 2개를 연산할 때에 하나가 문자열이면 나머지 하나도 문자열로 인식합니다. (뺄셈과 나눗셈, 곱셈의 방식은 다릅니다.)
    ```js
    console.log("2" + 2); // 22
    console.log(2 + "2"); // 22
    console.log(2 + 2 + "1"); // 41
    // 덧셈이 아닌 경우에는 다른 결과가 나옵니다 (숫자처럼 인식)
    console.log(2 * "2"); // 4
    console.log("2" * "2"); // 4
    console.log("2" * 2); // 4
    ```
